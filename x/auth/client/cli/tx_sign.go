...
func KycCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:    "kyc [address]",
		Short:  "Pass the address if kyc is done already",
		Long:   `Pass the address if kyc is done already`,
		PreRun: preSignCmd,
		RunE:   makeKycCmd(),
		Args:   cobra.MinimumNArgs(1),
	}

	cmd.Flags().String(flagKyc, "", "Address")
...
func makeKycCmd() func(cmd *cobra.Command, args []string) error {
	return func(cmd *cobra.Command, args []string) error {
		fmt.Println(args)
		return nil
	}
