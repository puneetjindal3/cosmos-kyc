...
type AccountKeeperI interface {
	...
	// Set user kyc
	SetKyc(context.Context, sdk.AccAddress) bool
}
...
// Set Kyc return bool value that it will accept the tx or not
func (ak AccountKeeper) SetKyc(ctx context.Context, addr sdk.AccAddress) bool {
	msg := msgServer{
		ak: ak,
	}
	kycData := &types.KycRequest{
		Address: addr.String(),
	}
	kStatus, _ := msg.SetKyc(ctx, kycData)
	if kStatus.Status {
