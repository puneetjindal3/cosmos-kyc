...
func (ms msgServer) SetKyc(goCtx context.Context, msg *types.KycRequest) (*types.KycResponse, error) {
	kvStorA := ms.ak.storeService.OpenKVStore(goCtx)
	err := kvStorA.Set([]byte(msg.Address), []byte("true"))
	if err != nil {
		return &types.KycResponse{Status: false}, nil
	}
	return &types.KycResponse{Status: true}, nil
}
